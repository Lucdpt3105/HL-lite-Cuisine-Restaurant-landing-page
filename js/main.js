              ...
              if (!deferred) {
                ...
                (deferred = $.Deferred()),
                  (resolve = function () {
                    setTimeout(function () {
                      deferred &&
                        (li.data("hovering.dropdown") || deferred.resolve());
                    }, options.delay);
                  });
...
function initNavigation() {
  ...
  $('.nav-menu a[href^="#"], .dropdown-menu a[href^="#"]').on(
    "click",
    function (e) {
      e.preventDefault();
...
function initScrollEffects() {
  ...
  document
    .querySelectorAll(
      ".section-header, .chef-card, .seller-card, .service-item, .about-restaurant-section, .chefs-section, .menu-section, .top-sellers-section, .events-section, .contact-section"
    )
    .forEach((el) => {
      observer.observe(el);
    });
...
function showOrderConfirmation(dishName) {
  ...
  if (confirmationMsg.length === 0) {
    ...
    confirmationMsg
      .find("p")
      .html(`<strong>${dishName}</strong> has been added to your order.`);
  }
...
function showMenuOrderConfirmation(dishName, price) {
  ...
  if (confirmationMsg.length === 0) {
    ...
    confirmationMsg
      .find("p")
      .html(
        `<strong>${dishName}</strong> (${price}) has been added to your order.`
      );
  }
...
function initEventsCarousel() {
  ...
  function restartAnimations(card) {
    const animatedElements = card.find(
      ".event-category-badge, .event-title, .event-date-text, .event-description, .event-booking-btn"
    );
...
function initContactForm() {
  $("#reservationForm").on("submit", function (e) {
    ...
    setTimeout(function () {
      ...
      showMessage(
        `Thank you, ${formData.name}! Your reservation request has been submitted. We'll contact you shortly to confirm your booking for ${formData.date} at ${formData.time} for ${formData.guests} guests.`,
        "success"
      );
    }, 2000);
  });
...
function initBookingSystem() {
  ...
  $("#guestNameForm").on("submit", function (e) {
    ...
    if (guestName) {
      ...
      setTimeout(() => {
        alert(
          `Welcome, ${guestName}! You can now browse our menu and add items to your cart.`
        );
      }, 300);
    }
  });
  ...
  $(".booking-option-btn").on("click", function () {
    ...
    if (bookingType === "seats-only") {
      ...
      bookingType === "seats-with-food" ||
      bookingType === "celebration"
    ) {
  ...
  function updateCart() {
    ...
    if (cart.length === 0) {
      cartItemsContainer.html(
        '<p style="text-align: center; color: #999; padding: 2rem;">Cart is empty</p>'
      );
    } else {
  ...
  function updateCartModal() {
    ...
    if (cart.length === 0) {
      cartModalItems.html(
        '<p style="text-align: center; color: #999; padding: 40px;">Your cart is empty. Browse our menu to add items!</p>'
      );
      ...
      const cartItemHtml = `
                <div class="cart-modal-item">
                    <div class="cart-modal-item-info">
                        ...
                        <span class="cart-modal-item-price">$${item.price.toFixed(
                          2
                        )} each</span>
                    </div>
                    ...
                    <div class="cart-modal-item-total">$${itemTotal.toFixed(
                      2
                    )}</div>
                </div>
            `;
  ...
  $(".payment-btn").on("click", function () {
    ...
    alert(
      `Payment method selected: ${paymentType}\nTotal: ${$(
        "#cartTotal"
...
$(".newsletter-form").on("submit", function (e) {
  ...
  if (email && email.includes("@")) {
    alert(
      "Thank you for subscribing! 🎉\nYou will receive our exclusive offers at " +
        email
    );
